 ...
 *
 */
/* global jQuery, BaForm, tinymce, Joomla, modName,ajaxurl,wp */
/* eslint no-var: off */
; (function (win, doc, $) {
  // BEGIN FUNCTION ///////////////////
  'use strict'
  ...
  $.fn.oMain = function () {
    return this.each(function () {
      ...
      var disableArray = ['subheader', 'image', 'position', 'align', 'effect', 'nextPrev', 'autoPlay', 'autoplayDelay', 'pauseOnHover', 'keyNav', 'breakPoint', 'breakPointWidth', 'styleMode', 'background', 'color', 'borderColor', 'fontFamily', 'fontSize', 'fontWeight', 'textTransform', 'icon', 'margin', 'padding', 'borderStyle', 'borderWidth', 'borderRadius']
  ...
  $(doc).ready(function () {
    var applyButton = doc.querySelector('.button-apply, #publishing-action > [name="save"]')
    ...
    applyButton.onclick = function () {
      ...
      if (formBasic.length > 0) {
        ...
        bf.oSetJson(jsonData)
        setTimeout(function () {
          win.Joomla ? Joomla.submitbutton('module.apply') : $(applyButton).closest('form').submit()
        }, 100)
      }
    }
  ...
  function runAjax () {
    ...
    if (typeof ajaxurl !== 'undefined') { // FOR Wordpress
      $.post(ajaxurl, { action: 'ba_preview', id: $('#post_ID').val(), 'ba-form-content': baFormContent }, function (response) {
        previewPopup(response)
      })
    } else { // FOR Joomla
    ...
    function previewPopup (response) {
